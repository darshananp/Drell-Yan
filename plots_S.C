...

{
 ...
 TCut RTrig ("(lvl1_trigscaled&0x00010000)>0");
  ...
  cut_dy_pm = EvertexCut  && charge0_cut  && massCut && muonCuts && cut_eta && armCut_S ;// && fvtxCuts_2 ;
  cut_bb_pm = EvertexCut  && charge0_cut  && massCut && muonCuts && cut_eta && armCut_S && bb_cut;// && fvtxCuts_2;
  cut_cc_pm = EvertexCut  && charge0_cut  && massCut && muonCuts && cut_eta && armCut_S && cc_cut;// && fvtxCuts_2;
  cut_rd_pm = EvertexCut  && charge0_cut  && massCut3 && muonCuts && cut_eta && armCut_S  && RTrig;// && fvtxCuts_2;
  cut_rd_pp = EvertexCut  && charge2_cut  && massCut && muonCuts && cut_eta && armCut_S  && RTrig;// && fvtxCuts_2;
  cut_rd_mm = EvertexCut  && charge3_cut  && massCut && muonCuts && cut_eta && armCut_S  && RTrig;// && fvtxCuts_2;

  cut_bb_pp = EvertexCut  && charge2_cut  && massCut && muonCuts && cut_eta && armCut_S && bb_cut;// && fvtxCuts_2;
  cut_bb_mm = EvertexCut  && charge3_cut  && massCut && muonCuts && cut_eta && armCut_S && bb_cut;// && fvtxCuts_2;
  cut_pr_pm = EvertexCut  && charge0_cut  && massCut2 && muonCuts && cut_eta && armCut_S;// && fvtxCuts_2 ;
...
TMatrixDSym cov_pr = r_pr->GetCovarianceMatrix();
...
cout<<"Integral of template for pr: "<<    f_2d_pr->Integral(3.75,8,0,50,1E-5)/0.25<<"\n" 
    <<"Error of the fit for pr    : "<<    f_2d_pr->IntegralError(2,a,b,f_2d_pr->GetParameters(), cov_pr.GetMatrixArray())/0.25<<"\n";
...
int lo_binx = h_2d_final_dy->GetXaxis()->FindBin(4.50001);
int hi_binx = h_2d_final_dy->GetXaxis()->FindBin(4.99999);
int lo_biny = h_2d_final_dy->GetYaxis()->FindBin(15.00001);//5.00001,10.00001,15.00001
int hi_biny = h_2d_final_dy->GetYaxis()->FindBin(49.9999);//9.9999,14.9999,49.9999
...
int lo_binx_pp = h_2d_final_dy->GetXaxis()->FindBin(8.50001);
int hi_binx_pp = h_2d_final_dy->GetXaxis()->FindBin(8.99999);
int lo_biny_pp = h_2d_final_dy->GetYaxis()->FindBin(65.00001);//50.001,55.00001,60.0001,65.0001
int hi_biny_pp = h_2d_final_dy->GetYaxis()->FindBin(99.9999);//54.999,59.9999,64.999,99.999
...
int lo_binx_mm = h_2d_final_dy->GetXaxis()->FindBin(12.50001);
int hi_binx_mm = h_2d_final_dy->GetXaxis()->FindBin(12.99999);
int lo_biny_mm = h_2d_final_dy->GetYaxis()->FindBin(115.00001);//100.001,105.00001,110.0001,115.001
int hi_biny_mm = h_2d_final_dy->GetYaxis()->FindBin(149.9999);//104.999,109.9999,114.999,149.000

...
void Fun4AllFVTX_ideal(
  int nEvents = 0,
  bool use_svx_cluster = false,
  const char *inputfile = "PISAEvent.root",
  const char *evtgen = "phpythia.root",
  const char *dstfile   = "dst_pisa_ideal.root",
  const char *dstreaderfile = "dst_reader_ideal.root",
  const char *inputfile2 = "/phenix/u/jinhuang/links/fvtx_tmp/miliped_work/Filter_Clock/DST.list", 
  const char *ana_file = "muon_ana_ntuples_ideal.root",
  const char *eval_file = "mutoo_eval_ntuple_ideal.root",
  const char *singlepdstout = "singlemuon_pdst_ideal.root",
  const char *dimuonpdstout = "dimuon_pdst_ideal.root",
...

{
  ...
  bool use_perfect_detector = true;
...
// This includes the MUID tube efficiencies
  ...
  bool use_local_hv_files = false;
	if( use_local_hv_files )
	{
		...
		hv_file << "/direct/phenix+spin2/darshana/Run_Condor_Setups/MUTHVFILES/R13/mut.disabledAnodes.dat_run" << run_number;

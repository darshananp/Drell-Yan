...
void
Fun4FVTX_mass(int nEvents = 100000,char *input_file = "afterqa.list", char *dst_file = "dst_out.root")
{
 ...
 // gSystem->Load("libspin_analyzer");
  ...
  sa->set_auto_save_hist(
      string(input_file) + string("_") + string("mass_hist.root"));
  sa->Verbosity(1);

  sa->RegisterModule(new saModuleDimuonDarshana());
